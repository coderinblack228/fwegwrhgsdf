	...
	local library = loadstring(game:HttpGet'https://raw.githubusercontent.com/Cr1st1cal/serenity/main/message%20(4).txt')()
			local Watermark = "test"
			-- set this ^ to whatever build this is
			if Watermark == "test" then
				library:CreateWatermark("suka.blyat | suka.blyat always on top 1")
				library.theme.accentcolor = Color3.fromRGB(0, 0, 0)
				library.theme.accentcolor2 = Color3.fromRGB(0, 0, 0)
				library.theme.topheight = 10
				library.theme.TextColor = Color3.fromRGB(0, 0, 0)
			end
			if Watermark == "Dev" then
				library:CreateWatermark("suka.blyat | Build: Dev")
				library.theme.accentcolor = Color3.fromRGB(0, 0, 0)
				library.theme.accentcolor2 = Color3.fromRGB(0, 0, 0)
				library.theme.topheight = 45
				library.theme.TextColor = Color3.fromRGB(0, 0, 0)
			end
			if Watermark == "Beta" then
				library:CreateWatermark("suka.blyat | Build: Beta")
				library.theme.accentcolor = Color3.fromRGB(0, 0, 0)
				library.theme.accentcolor2 = Color3.fromRGB(0, 0, 0)
				library.theme.topheight = 45
				library.theme.TextColor = Color3.fromRGB(0, 0, 0)
			end
			if Watermark == "Public" then
				library:CreateWatermark("suka.blyat | Build: Public")
				library.theme.accentcolor = Color3.fromRGB(0, 0, 0)
				library.theme.accentcolor2 = Color3.fromRGB(0, 0, 0)
				library.theme.topheight = 45
				library.theme.TextColor = Color3.fromRGB(0, 0, 0)
	...
	local cfglocation = "suka.blyat_configs/"
	makefolder("suka.blyat_configs")
	...
	function library:ConfigFix(cfg)
		local copy = game:GetService("HttpService"):JSONDecode(readfile(cfglocation..cfg..".cfg"))
	...
	function library:SaveConfig(cfg)
		...
		writefile(cfglocation..cfg..".cfg", game:GetService("HttpService"):JSONEncode(copy))
	end
	...
	function library:New(name)
		...
		do
			...
			TextLabel.BorderColor3 = Color3.fromRGB(169, 235, 255)
			TextLabel.Position = UDim2.new(0, 1, 0.300000012, 0)
			TextLabel.Size = UDim2.new(0, 155, 0, 24)
			TextLabel.ZIndex = 2
			TextLabel.Font = Enum.Font.SourceSansSemibold
			TextLabel.Text = "gaybindz"
			TextLabel.TextColor3 = Color3.fromRGB(169, 235, 255)
				...
				function Sector:Element(type, text, data, callback)
					...
					elseif type == "TextBox" then
						...
						TextBox:GetPropertyChangedSignal("Text"):Connect(function()
							if string.len(TextBox.Text) > 40 then
								TextBox.Text = string.sub(TextBox.Text, 1, 40)
							end
		...
		local gui = library:New("suka.blyat")      
		...
		antiaim:Element("Dropdown", "pitch", {options = {"None", "Up", "Down","Huge", "Custom", "Zero", "180", "Random AA", "suka.blyat", "God", "Sus_down", "wtf", "glitch", "sussy", "fucker"}})   
		...
		local spammessages = {
			...
			"1 Corinthians 15:58. Therefoce, my beloved brothers, be steadfast, immovable, always abounding in the work of the",
			"Deuteronomy 31:6 Be strong and courageous. Do not be afraid or terrified because of them, or the LORD your God goes with",
			"Psalm 27:12 The LORD is my light and my salvation; whom shall i fear? The LORD is the stronghold of my life; of whom",
			"Romans 8:31 What, then, shall we say in response to there things? If god is for us, who can be against us?",
		...
		chat:Element("Toggle", "chat spam", nil, function(tbl)
			if tbl.Toggle then
				while values.misc.chat["chat spam"].Toggle do
					game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Standard" and spammessages[math.random(1,table.getn(spammessages))] or cwspammsg[math.random(1,table.getn(cwspammsg))], false, "Innocent", false, true)
					wait(values.misc.chat["speed (ms)"].Slider/1000)
				end
			end
		end)
		chat:Element("Dropdown", "type", {options = {"Standard", "cuteware"}})
		...
		RunService.RenderStepped:Connect(function(step)     
			...
			if PlayerIsAlive then      
				...
				if values.rage.angles.enabled.Toggle and not DisableAA then      
					...
					local Pitch = values.rage.angles["pitch"].Dropdown == "Sus_down" and -3 or values.rage.angles["pitch"].Dropdown == "None" and CamLook.Y or values.rage.angles["pitch"].Dropdown == "suka.blyat" and -129433 or values.rage.angles["pitch"].Dropdown == "Up" and 1 or values.rage.angles["pitch"].Dropdown == "Down" and -1.3 or values.rage.angles["pitch"].Dropdown == "Zero" and 0 or values.rage.angles["pitch"].Dropdown == "Random AA" and RANDOM(-99999999, 100)/100 or values.rage.angles["pitch"].Dropdown == "Huge" and math.huge or values.rage.angles["pitch"].Dropdown == "Custom" and values.rage.angles["pitch angle"].Slider or 2.5 or values.rage.angles["pitch"].Dropdown == "wtf" and 99999999999999999999999999999999999999999999999999999999 or values.rage.angles["pitch"].Dropdown == "glitch" and (0 + -5) or values.rage.angles["pitch"].Dropdown == "God" and 15 or values.rage.angles["pitch"].Dropdown == "fucker" and -13 or values.rage.angles["pitch"].Dropdown == "sussy" and math.huge -1/0 -5  
		...
		LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
			...
			if values.misc.chat["kill say"].Toggle then      
				game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "Wow you got killed by suka.blyat ❤️😏. I love you at 105 °C. Drops of pure distilled water", false, "Innocent", false, true)      
			end      
		end)      
